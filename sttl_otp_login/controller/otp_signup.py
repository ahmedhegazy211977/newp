...
class OtpSignupHome(Home):
    ...
    def web_signup_otp(self, **kw):
        ...
        if "mobile" in qcontext and qcontext["mobile"] and "login" in qcontext and qcontext["login"] and qcontext["password"] == qcontext["confirm_password"]:
            qcontext['providers']={}
            ...
            if user_id:
                ...
                mobile = str(qcontext.get('mobile'))
                login = str(qcontext.get('login'))
    ...
    def web_otp_signup_verify(self, *args, **kw):
        ...
        try:
            ...
            if otp_no == otp:
                ...
                response = request.render('sttl_otp_login.custom_otp_signup', {'otp': True, 'otp_login': True,                                                           'confirm_password': confirm_password})
                return response
        except UserError as e:
            qcontext['error'] = e.name or e.value

        response = request.render('sttl_otp_login.custom_otp_signup', {'otp': True, 'otp_login': True,
                                                                       'login':login,'mobile': mobile,'name': name,
                                                                       'password': password,
